(function () {
            ...
            image.onload = function () {
              ...
              if (isBuilder) {
                ...
                $(document).on("delete.cards", function (event) {
                  player.stopResize();
                  player.stopReplay(
                    event.target.querySelector(".mbr-background-video-wrapper")
                  );
                });
                $(document).on(
                  "changeParameter.cards",
                  function (event, paramName, value, key) {
                    const bgVideo = event.target.querySelector(
    ...
    if (!isBuilder) {
      document.addEventListener("click", function (e) {
        try {
          var target = e.target;
          if (
            target.parents().some((el) => el.classList.contains("carousel"))
          ) {
            return;
          }
          do {
            if (target.hash) {
              ...
              document
                .querySelectorAll(useBody ? "body" : target.hash)
                .forEach(function (el) {
                  e.preventDefault();
  ...
  if (!isBuilder) {
    ...
    const dropdownTogglesArray = document.querySelectorAll(
      ".dropdown-toggle.show"
    );
    const dropdownMenusArray = document.querySelectorAll(
      ".dropdown-menu.show, .dropdown.open"
    );
    ...
    dropdownMenusArray.forEach((dropdownMenuItem) =>
      dropdownMenuItem.classList.remove("show")
    );
  ...
  function initTestimonialsCarousel(card) {
    var target = card,
      ...
      bs5 =
        ...
        target.getAttribute("data-bs-version").startsWith("5");

    if (target.getAttribute("id") === null) {
      carouselID =
        target.classList.value.match(/cid-.*?(?=\s|$)/) + "-carousel";
    }
...
!(function () {
  ...
  if (!document.getElementById("top-1")) {
    ...
    a.innerHTML =
      '<a href="https://mobirise.com/qr/best-qr-code-generator.html">Best Free QR Code Generator</a> Mobirise v5.6.4 <a href="https://mobirise.com?u=552443"> Best Website Building Software</a>';
