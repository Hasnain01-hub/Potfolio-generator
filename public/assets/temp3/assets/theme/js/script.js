(function ($) {
      ...
      if ($.isMobile()) {
        ...
        function elCarouselItem(element) {
          ...
          if (parentEl.find(".carousel-item.active .hidden.animated").lenght) {
            ...
            if (
              element.parents().is(".cloneditem-" + (visibleSlides - 1)) &&
              element
                .parents(".cloneditem-" + (visibleSlides - 1))
                .attr("data-cloned-index") >= visibleSlides
            ) {
              return true;
            } else {
  ...
  $(document).on("change", 'input[type="range"]', function (e) {
    $(e.target).parents(".form-group").find(".value")[0].innerHTML =
      e.target.value;
  });
})(jQuery);
